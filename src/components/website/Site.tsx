...
export default function Site({initial,catalog}:{initial:WebsiteContent;catalog:SiteCatalog}) {
  ...
  return <div style={style} className={c.identity.rounded?'editable-site rounded-site':'editable-site'}>
    ...
    <main>
      ...
      {c.services.enabled&&<section id="services" className="services-section section-pad"><div className="section-heading"><div><p className="eyebrow">TRATAMIENTOS</p><h2>{c.services.title}</h2></div></div><div className="service-grid">{c.services.cards.map((s,i)=><article key={s.id} className="service-card"><div className="service-photo"><img src={s.image} alt={s.alt} className="cover-image" loading="lazy"/></div><div className="service-meta"><span>{String(i+1).padStart(2,'0')} / TRATAMIENTOS</span><span>↗</span></div><h3>{s.title}</h3><p>{s.description}</p><a href="#reservas">{c.services.linkText} →</a></article>)}</div></section>}
      ...
      <Booking title={c.booking.title} intro={c.booking.intro} preview={preview}/>
